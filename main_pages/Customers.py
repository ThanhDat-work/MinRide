...
from TinhNang import Customer

class CustomersPage(tk.Frame):
    def __init__(self, parent, logo_img, csv_path="data/customers.csv"):
        ...
        self.upd_district = tk.StringVar()
        ...
        # list chứa kết quả tìm kiếm theo quận
        self.show_all_district =False
        self.lst_district=[]
        ...
        # ===== In-memory data (demo) =====
        self.repo1=CustomerManager("data/customers.csv")
        self.repo1.sort_lst_ID()
        try:
            self.next_id= self.repo1.get_id_lastCustomer()+1
        except:
            self.next_id=1
    ...
    def _build_topk_panel(self, parent):
        wrap = self._panel_wrap(parent, "Top K khách hàng (đầu/cuối)")
    ...
    def _topk_show(self):
        ...
        out = []
        for c in pick:
            out.append(f'ID: {c.id} | Tên: {c.name} | Quận: {c.district} \nTọa độ: ({c.x},{c.y})\n')
        self.result_var.set("\n".join(out) if out else "(trống)")
        self.msg_var.set(f"Đã hiển thị {len(pick)} khách hàng.")
        self.draw_ui()

    # ---- Add ---- (hoàn chỉnh)
    def _build_add_panel(self, parent):
        wrap = self._panel_wrap(parent, "Thêm khách hàng")
        self.add_name = tk.StringVar()
        self.add_district = tk.StringVar()
        self.add_x = tk.StringVar()
        self.add_y = tk.StringVar()

        self._field(wrap, "Họ tên:", self.add_name)
        self._field(wrap, "Quận:", self.add_district)
    ...
    def _add_submit(self):
        ...
        district = self.add_district.get().strip()
        x = float(self.add_x.get().strip())
        y = float(self.add_y.get().strip())

        if not name or not district or not x or not y:
            self.msg_var.set("Vui lòng nhập ít nhất: Họ tên, Quận, Tọa độ x, Tọa độ y.")
            self.draw_ui()
            return
        ...
        self.repo1.add_customer(customer)
        self.next_id+=1
        self.msg_var.set(f"Đã thêm khách hàng ID {self.next_id - 1}.")

        # reset form
        self.add_name.set("")
        self.add_district.set("")
        self.add_x.set("")
        self.add_y.set("")
    ...
    def _build_update_panel(self, parent):
        wrap = self._panel_wrap(parent, "Cập nhật khách hàng theo ID")
        ...
        self._field(wrap, "Quận", self.upd_district)
    ...
    def _update_load(self):
        ...
        if not c:
            self.upd_name.set("")
            self.upd_district.set("")
            self.upd_x.set("")
            self.upd_y.set("")
            self.msg_var.set("Không tìm thấy khách hàng.")
            self.draw_ui()
            return
        self.upd_name.set(c.name)
        self.upd_district.set(c.district)
        self.upd_x.set(c.x)
        self.upd_y.set(c.y)
    ...
    def _update_submit(self):
        ...
        new_district = self.upd_district.get().strip() or old_c.district
        ...
        self.repo1.update_customer(temp_customer)
    ...
    def _build_delete_panel(self, parent):
        wrap = self._panel_wrap(parent, "Xóa khách hàng theo ID")
    ...
    def _delete_submit(self):
        ...
        ok = self.repo1.delete_customer(cid)
        if ok:
    ...
    def _build_search_panel(self, parent):
        wrap = self._panel_wrap(parent, "Tìm kiếm khách hàng (Tên hoặc ID)")
        ...
        tk.Button(wrap, text="Tìm", bg="#016B61", fg="white", bd=0,
                  font=("Segoe UI", 10, "bold"),
                  command=self._search_customer).pack(anchor="e", pady=(10, 0))
    ...
    def _search_customer(self):
        ...
        try:
            ...
            cid = None
        repo=CustomerManager("data/customers.csv")
        data=repo.load_from_csv()
        for c in data:
            if cid is not None:
                if c.id == cid:
                    out.append(f'ID: {c.id} | Tên: {c.name} | Quận: {c.district} \nTọa độ: ({c.x},{c.y})\n')
    ...
    def _build_district_panel(self, parent):
        wrap = self._panel_wrap(parent, "Liệt kê theo Quận")
        self.district_name_var = tk.StringVar(value=self.district_name_var.get() if hasattr(self, 'district_name_var') else "")
        
        # Dòng nhập tên quận và nút Tìm
        input_row = tk.Frame(wrap, bg="white")
        input_row.pack(fill="x", pady=5)
        
        self._field(input_row, "Tên quận:", self.district_name_var)
        
        # Sửa lỗi command ở đây bằng lambda
        tk.Button(wrap, text="Tìm kiếm", bg="#016B61", fg="white", bd=0,
                font=("Segoe UI", 10, "bold"),
                command=lambda: self._district_check()).pack(anchor="e", pady=5)

        # Box hiển thị kết quả
        self._result_box(wrap)

        # Nút "Xem thêm" chỉ hiện khi có nhiều hơn 10 người và chưa nhấn Xem thêm
        if len(self.lst_district) > 10 and not self.show_all_district:
            tk.Button(wrap, text="Xem thêm kết quả...", bg="#016B61", fg="white",
                    bd=0, font=("Segoe UI", 10, "bold"),
                    command=self._handle_view_more).pack(pady=5)
        
        return wrap

    def _district_check(self):
        q = self.district_name_var.get().strip().lower()
        if not q:
            self.msg_var.set("Vui lòng nhập tên quận.")
            self.draw_ui()
            return
        ...
        repo = CustomerManager("data/customers.csv")
        all_customers = repo.load_from_csv()
        
        # 3. Lọc (Bạn nên Sort ID ở đây để đúng yêu cầu đề bài)
        matches = [c for c in all_customers if c.district.lower() == q]
        matches.sort(key=lambda x: x.id) # Sắp xếp ID tăng dần
        
        for c in matches:
            self.lst_district.append(f"ID: {c.id} | Tên: {c.name} \nQuận: {c.district} ({c.x}, {c.y})\n")

        # 4. Cập nhật hiển thị
        total = len(self.lst_district)
        if total == 0:
            self.result_var.set("Không tìm thấy khách hàng nào.")
        else:
            # Chỉ lấy 10 người đầu tiên cho lần đầu
            display_list = self.lst_district[:10]
            header = f"Tổng cộng: {total} khách hàng tại quận {q.upper()}\n"
            header += f"Đang hiển thị 10 khách hàng đầu tiên:\n" + "-"*30 + "\n"
            self.result_var.set(header + "\n".join(display_list))
            
        self.msg_var.set(f"Tìm thấy {total} kết quả.")
        self.draw_ui()

    def _handle_view_more(self):
        """Xử lý khi nhấn nút Xem thêm"""
        self.show_all_district = True
        total = len(self.lst_district)
        q = self.district_name_var.get().upper()
        
        header = f"Tổng cộng: {total} khách hàng tại quận {q}\n"
        header += f"Hiển thị toàn bộ danh sách:\n" + "-"*30 + "\n"
        
        self.result_var.set(header + "\n".join(self.lst_district))
        self.draw_ui()
    # ================== DRAW UI ==================
    def draw_ui(self, event=None):
        ...
        # background
        TinhNang.Background(cv, "#80D7CB", "#C7E3DF")

        # header
        cv.create_text(pad, 56, text="Customers",
        ...
        if self.expanded_key and expanded_row is not None:
            ...
            elif self.expanded_key == "District":
                content = self._build_district_panel(inner)
            else:
